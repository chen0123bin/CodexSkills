...
随着工作流推进，再逐步创建当前版本下的 `prd.md`、`plan.md`、`task.md`、`progress.md` 和 `delivery.md`。
...
1. 完成当前 task 描述要求的实现
2. 如果缺少信息或遇到真实技术阻塞，将 task 标记为 `blocked`
3. 如用户明确要求委派或并行处理，再额外参考 `references/subagents.md`
...
5. 更新 `task.md`，将当前 task 标记为 `done` 并记录完成时间
6. 更新 `progress.md`
...
- 如果还有 `pending` task，回到 Step 1
- 如果存在 `blocked` task 且没有更多 `pending` task，向用户汇报阻塞情况
- 如果所有 task 都已完成，进入 Phase 4

### 异常处理

- 验证失败时，先尝试自动修复一次；仍失败则标记为 `blocked`
