...
- PRD 阶段的三方分析：用户明确提到 Debate、subagent、CTO/CPO/CMO、委派、并行分析，或在主代理询问后明确同意
- 执行阶段的开发与审查：用户明确要求使用子代理、delegation、并行处理或多人分工

如果用户没有明确授权真实子代理，默认由主代理执行。PRD 阶段在用户拒绝真实分析子代理时，仍保留相同文件结构，但改为主代理串行完成三份角色分析。
...
主代理在 PRD 阶段先完成需求自分析与最多三轮问答，再负责编排三方分析与裁决收拢：

1. 先读取 `docs/project-memory.md`，再做详细需求分析，识别需要确认的关键问题
2. 按需生成 1 到 3 轮 `round-{N}.md` 并等待用户回答
3. 决定 Step 2 使用真实子代理还是单代理串行 fallback
4. 向 `cto`、`cpo`、`cmo` 分发相同的已完成 round 文档
5. 等待三份分析全部完成
6. 提取共识、分歧和折中空间
7. 生成 `analysis-summary.md`
8. 向用户呈现综合方案和推荐意见
9. 根据用户选择写入 `consolidated.md`
10. 关闭已完成的分析 agent thread

### 分析输入上下文

三个分析角色只接收已完成的 round 文档，例如：
...
- 从技术可行性、架构适配、复杂度、可扩展性和技术债角度独立分析
