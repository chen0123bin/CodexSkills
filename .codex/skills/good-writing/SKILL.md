...
- [ ] **AI「判断式/定义式」陈述**：正文是否出现 `A 是 B`（定性等价判断，如"民粹主义是一种政治策略"）、`A 不是 B`（单独的否定等价判断，"A 不是 B 而是 C"已由第 21 条与 9.1 节覆盖）、`所谓 A 即 B` / `A 就是 B` / `A 本质上是 B` / `A 意味着 B` / `A 可以概括为 B`（字典式定义）的密集堆叠？是否滥用 `必然/显然/毋庸置疑/根本/绝对/无疑/一定/无可争议` 等绝对化副词修饰判断句？全文合计是否 `<= 3`？每处是否紧跟机制、事实或限定条件（不得悬空下论断）？作者下"重判决"的情绪峰值句（如"所以我说他无耻。"）是否紧接可核对事实？命中即改——把等价判断改为动作/事实句，把否定壳改为让步-推进句，把定义式改为功能/场景还原，把绝对副词改为限定性表述
...
29. **禁止 AI「判断式/定义式」陈述的机械堆叠与绝对化语气**：这是 LLM 模拟"权威解释"时最隐蔽也最高频的统计输出之一——把本该由事实和推理自然浮现的结论，直接压缩成一句等价判断或字典式定义，读感像辞典开条或维基首句。原作者的"冷判决"笔法看上去也在下判断，但两者的根本差别在于：真人作者的判断是从事实堆叠中自然收束出来的（"所以我说他无耻。"——前面已给足事实），AI 的判断是先悬空下论断再找支撑（"民粹主义是一种政治策略。"——后面才补细节）。命中目标：
    - **定性等价判断**：`A 是 B`（作为定性标签使用，非事实陈述）——如"民粹主义是一种政治策略。"
    - **否定等价判断**：`A 不是 B`（单独使用；配合"而是 C"已由第 21 条"否定转折模板"与 `ai-trace-detector.md` 9.1 节"二分对照壳"覆盖，此处针对单独出现的否定等价句）
    - **字典式定义**：`所谓 A 即 B` / `A 就是 B` / `A 本质上是 B` / `A 意味着 B` / `A 可以概括为 B`——如"所谓觉醒即认清现实。"
    - **绝对化副词修饰的论断**：`必然/显然/毋庸置疑/根本/绝对/无疑/一定/无可争议` + 判断句——如"他们根本不懂学术。"
    - **执行标准**：正文中上述句式（合并计算）全文 `<= 3`，且每处必须紧跟机制、事实或限定条件；判断必须落在可核对的事实或已展开的论证之后，不得悬空下论断
    - **豁免条件**：原作者下"重判决"的情绪峰值句允许保留 1 处（如"所以我说他无耻。"），但该句必须紧接或承接可核对事实与论证链；直接引语提示语后的原话保留原样
    - **改写策略**：
      - 把 `A 是 B` 改为动作或事实描述（`A 在做 X` / `A 表现出 X` / `A 呈现 X 的样子`）
      - 把 `A 不是 B` 改为不对称对照（`A 固然带着 B 的某些特征，但……` / `A 远不止 B 这么简单`），避免工整否定壳
      - 把 `所谓 A 即 B` 改为功能/场景还原（`碰上 A 的时候，常见的做法是……` / `A 这个说法，多半指的是……`）
      - 把绝对化副词换成限定性表述（`多半` / `通常` / `至少在……情况下` / `在可见的范围内` / `看起来` / `目前为止`）
    - **正误对照**：
      - ❌ "民粹主义是一种政治策略。"（悬空下论断，无机制无事实）
      - ✅ "民粹主义多半在两种场景里冒头：旧秩序失信的时候，以及经济上升期被打断的时候。"（场景还原替代标签）
      - ❌ "AI 不是工具，而是生产力。"（二分对照壳 + 等价判断双重模板）
      - ✅ "AI 在某些场景里更像工具，可一旦它介入创作与决策，'工具'这个词就不够用了。"（场景限定 + 推进）
      - ❌ "所谓觉醒即认清现实。"（字典式定义）
      - ✅ "人讲'觉醒'这个词的时候，多半是在给自己的不满找一个体面的说法。"（还原使用场景）
      - ❌ "他们根本不懂真正的学术。"（绝对副词 + 空判断）
      - ✅ "他们至少在论证链条和引证规范上站不住，这两条是学术的底线。"（限定 + 机制）
